...
from fedml_api.dynamic_pruning.init_scheme import generate_layer_density_dict, pruning
import warnings
import math

# dynamic pruning starts with a random sparse network, and at regularly spaced intervals it removes a fraction of connections 
# based on their magnitudes and activates new ones using instantaneous gradient information. After updating
# the connectivity, training continues with the updated network until the next update. The main parts of our algorithm,
#Sparsity Distribution, Update Schedule, Drop Criterion, Grow Criterion, and the various options considered for each,
# are explained below.
...
class SparseModel(nn.Module):
    def __init__(self, inner_model,
                 ...
                 delta_t: int= 10, #the number of iterations between sparse connectivity updates
                 t_end: int=100, # the iteration at which to stop updating the sparse connectivity
                 alpha: float=0.1, #the initial fraction of connections updated 
                 ):
    ...
    def f_decay(self,current_t):
         self.decay= 0.5*self.alpha*(1+math.cos(current_t*math.pi/self.t_end))
...
if __name__ == "__main__":
    ...
    for i in range(200): # total training step t 
        sparse_model.zero_grad()
        # Sample a batch Bt(xi,yi) ∼ Dataset to be continued
        x = torch.randn([32, 3, 32, 32]).cuda()
        y = sparse_model(x)

        if i % sparse_model.delta_t == 0 and i < sparse_model.t_end:
           # k = sparse_model.f_decay(i)* (1-S)*N # the number of connections to be updated
            sparse_model.mask_dict = pruning(sparse_model.inner_model, sparse_model.layer_density_dict, "magnitude")
            sparse_model.apply_mask()
