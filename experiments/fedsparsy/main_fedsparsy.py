...
from api.distributed.fedsparsy.FedSparsyAPI import FedML_init, FedML_FedSparsy_distributed
...
def add_args(parser):
    ...
    parser.add_argument("--loss_based_lr_schedule", action="store_true", help="Enable loss-bucket based client learning rate schedule")
    parser.add_argument("--loss_lr_0_1", type=float, default=0.01, help="Client LR when 0 <= batch loss < 1")
    parser.add_argument("--loss_lr_1_2", type=float, default=0.005, help="Client LR when 1 <= batch loss < 2")
    parser.add_argument("--loss_lr_2_3", type=float, default=0.001, help="Client LR when 2 <= batch loss < 3")
    parser.add_argument("--loss_lr_ge_3", type=float, default=0.0005, help="Client LR when batch loss >= 3")

    parser.add_argument("--epochs", type=int, default=1, metavar="EP", help="how many epochs will be trained locally")

    parser.add_argument("--comm_round", type=int, default=500, help="how many round of communications we shoud use")
    ...
    parser.add_argument("--growth_data_mode", type=str, default="batch", help=" the number of data samples used for parameter growth, option are [ 'random', 'single', 'batch', 'entire']" )
     # SparsyFed specific arguments
    parser.add_argument("--reparam_beta", type=float, default=1.25, help='Re-parameterization beta for SparsyFed (default: 1.25)')
    parser.add_argument("--reparam_per_step", action="store_true", help="Apply re-parameterization before every batch (default: once per round)")
    parser.add_argument("--reparam_clip", type=float, default=0.0, help='Clamp re-parameterized weights to [-c, c]; 0 to disable (default: 0)')
    parser.add_argument("--log_activation_sparsity", action="store_true", help="Log activation sparsity stats during training (only in sparse rounds)")
    parser.add_argument("--batch_metric_log_dir", type=str, default="./batch_metric_logs", help="Directory for per-batch metric logs")
    parser.add_argument("--train_loss_log_interval", type=int, default=20, help="Interval (in batches) for writing train loss logs")
    parser.add_argument("--act_sparsity_log_interval", type=int, default=20, help="Interval (in batches) for writing activation sparsity logs")
    parser.add_argument("--target_sparsity", type=float, default=0.9, help='Target sparsity ratio for SparsyFed activation pruning (default: 0.9)')
    parser.add_argument("--dense_start_rounds", type=int, default=1, help='Number of initial communication rounds forced to dense activation + full update upload (default: 1)')
    parser.add_argument("--server_lr", type=float, default=1.0, help='Server learning rate for SparsyFed (default: 1.0)')
    parser.add_argument("--server_clip_norm", type=float, default=-1.0, help='Global update clip norm on server, set <=0 to disable (default: disabled)')
    parser.add_argument("--server_optimizer", type=str, default="sgd", help='Server optimizer for SparsyFed, options: [sgd, adam]')
    parser.add_argument("--server_momentum", type=float, default=0.0, help='Momentum used by the server SGD optimizer')
    parser.add_argument("--server_beta1", type=float, default=0.9, help='Beta1 used by the server Adam optimizer')
    parser.add_argument("--server_beta2", type=float, default=0.999, help='Beta2 used by the server Adam optimizer')
    parser.add_argument("--server_eps", type=float, default=1e-8, help='Numerical epsilon used by the server Adam optimizer')
...
def attach_sparse_reference_metadata(model, target_density, strategy):
    sparse_reference = SparseModel(copy.deepcopy(model), target_density=target_density, strategy=strategy)
    model.layer_density_dict = dict(sparse_reference.layer_density_dict)
    model.reference_mask_dict = {
        name: mask.detach().cpu().clone()
        for name, mask in sparse_reference.mask_dict.items()
    }
    logging.info("Reference sparse layers are %s", model.layer_density_dict)
    return model
...
if __name__ == "__main__":
    ...
    str_process_name = "FedSparsy(distributed):" + str(process_id)
    ...
    model = attach_sparse_reference_metadata(model, target_density=args.target_density, strategy=args.pruning_strategy)

    # start distributed training
    try:
        FedML_FedSparsy_distributed(
            process_id,
            worker_number,
            device,
            comm,
        ...
        if process_id == 0 and wandb.run is not None:
            wandb.finish()
