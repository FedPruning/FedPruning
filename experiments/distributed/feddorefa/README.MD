...
## Quick Start
```
CUDA_VISIBLE_DEVICES=0,1,2,3 sh run_feddorefa_distributed_pytorch.sh 100 10 resnet18 200 10 64 0.2 cifar10 0.5 5
```

## Usage
```
CUDA_VISIBLE_DEVICES=[gpus] sh run_feddorefa_distributed_pytorch.sh [client_num_in_total] [client_num_per_round] [model] [comm_round] [epochs] [batch_size] [initial_lr] [dataset] [partition_alpha] [frequency_of_the_test]
```
